...
const Profile = () => {
  ...
  const [coverColor, setCoverColor] = useState<string>(''); // State to store extracted color

  const coverRef = useRef<HTMLDivElement>(null); // Reference to cover div for color extraction
  ...
  useEffect(() => {
    if (profileData?.photo) {
      extractColor(profileData.photo); // Extract color when profile photo changes
    }
  }, [profileData?.photo]);
  ...
  const extractColor = (imageUrl: string) => {
    ...
    img.onload = () => {
      ...
      if (ctx) {
        ...
        // Calculate average color (simple algorithm)
        let r = 0,
          g = 0,
          b = 0;
    ...
    <DefaultLayout>
      ...
      <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="relative z-20 h-35 md:h-65">
          {coverColor && (
            <img
              src={generateColorImage(coverColor)}
              alt="profile cover"
              style={{
                width: '1500px',
                height: '260px',
